...
public:
    int minNumberOfHours(int initialEnergy, int initialExperience, vector<int>& energy, vector<int>& experience) {
        int energy_needed = std::accumulate(energy.begin(), energy.end(), 0);
        int energy_sum = (initialEnergy > energy_needed)? 0:energy_needed - initialEnergy + 1;
        int experience_res = 0;
        int experience_sum = initialExperience;
        for(int i = 0; i < experience.size(); i++) {
            if(experience_sum > experience[i]) {
                experience_sum += experience[i];
            } else {
                experience_res += experience[i] - experience_sum + 1;
                experience_sum = 2 * experience[i] + 1;
            }           
        }
        return experience_res + energy_sum;
    }
};
// @lc code=end
