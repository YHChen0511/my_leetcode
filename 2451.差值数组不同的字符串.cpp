 ...
 *
 * 给你一个字符串数组 words ，每一个字符串长度都相同，令所有字符串的长度都为
 * n 。
 *
 * 每个字符串 words[i] 可以被转化为一个长度为 n -
 * 1 的 差值整数数组 difference[i] ，其中对于 0 <= j <= n -
 * 2 有 difference[i][j] = words[i][j+1] -
 * words[i][j] 。注意两个字母的差值定义为它们在字母表中 位置
