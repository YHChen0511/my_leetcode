...
class Solution {
public:
  pair<long, long> check(const vector<long> &psum, const vector<long> &ppsum,
                         long mid, long n) {
  ...
  long getKSum(const vector<long> &psum, const vector<long> &ppsum, int k,
               int n) {
  ...
  int rangeSum(vector<int> &nums, int n, int left, int right) {
    ...
    return (int)((getKSum(psum, ppsum, right, n) -
                  getKSum(psum, ppsum, left - 1, n)) %
                 1000000007);
  }
};
// @lc code=end
